...
description: "Shion種族はHuman継承を維持し、Core所有の物理身体と公式DLCの公開機構で実装する。"
tags:
  - "shion"
  - "design"
  - "implementation"
...
Shion種族は、RimWorld 1.6本体と必須の公式DLCが提供するDef、Pawn renderer、健康、装備、仕事、Need、Gene、Xenotypeの公開機構を第一選択として実装する。
...
`Shion_ClockworkBody`は、食事不要、加齢停止、傷跡防止、Anomaly免疫を宣言するCore所有Defである。Defだけに公開hookがないCell喪失、休息下限、流体再処理停止、複製拒絶markerの維持は、同Geneを持つShion Pawn自身のtickだけで処理する。処理は対象Pawnと公式callbackの範囲に閉じる。

## 表現の拡張段階

`Shion_StandardBody`を採用し、内臓、狐耳、狐尾を物理的な健康対象にする。身体の具体的な器官、能力、欠損状態、心情は[Shion標準遠征身体の実装仕様](/design/56-Shion標準遠征身体の実装仕様.md)が所有する。

独自BodyDefを採用しても、`ThingDef ParentName="Human"`、標準Pawn renderer、apparel、仕事、健康tracker、手術Recipe、思考、Gene、Xenotypeは公開機構へ接続する。`Pawn_Shion`はAnomaly複製通知とspawn直後の拒絶だけを所有し、それ以外のPawn処理は基底`Pawn`へ委ねる。
...
Shion種族の型、描画、身体状態、保存データはCoreと公式DLCの公開機構が所有する。Vanilla Expanded FrameworkはFactory animation、完成Fleck、Endpoint overlayの投影範囲だけを所有する。
...
同一ビルドでは、新規生成、保存、ロード後にrace Def、Xenotype、Food Need不在、加齢係数、無性、身体部位、欠損効果、Cell喪失、Anomaly保護を維持する。`Shion_Race`のDef名を安定IDとして維持し、保存互換判断はdataVersionと公開段階に従う。
...
1. RimWorld 1.6、5公式DLC、VEF環境で正規パッケージをロードできる。
2. `About.xml`、Build props、C#、実行Defの依存と所有者が本節の採用構成に一致する。
...
7. 種族判定と身体処理が対象Pawnと公式callbackの範囲に閉じ、全Pawn走査を行わない。
