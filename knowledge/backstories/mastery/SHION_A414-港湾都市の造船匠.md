---
type: "Backstory Record"
title: "SHION_A414 港湾都市の造船匠"
description: "わしが最初に組んだ竜骨は、今も外港に係留されたまま現役で使われとる。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "canon"
eras:
  - "GrowthEra"
organization_groups:
  - "other-named"
organization_names:
  - "港湾都市"
  - "造船班"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A414"
canonical_scope: "backstory-mastery"
source_section: "SHION_A414 港湾都市の造船匠"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A414 港湾都市の造船匠

## 設定クラス

- Short: 造船匠
- Era: GrowthEra / Origin: 港湾都市 / Relation: 造船班

- Tone: 職人気質・郷愁・忍耐 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: Civilian
- VoiceProfile: 重々しく間を取る、経験を積んだ職人の口調。
- PotentialReferencePoints: 新規稿のため該当なし。

> わしが最初に組んだ竜骨は、今も外港に係留されたまま現役で使われとる。港が広がって新型の船を頼まれるたびに、古いやり方と新しい設計のどちらを信じるか迷うことがある。潮位標を見ていた子どもの頃の癖で、船を組む前には必ず港の潮を確かめるようにしとる。若い連中には縁起担ぎだと笑われるが、わしにはそれが一番確かな相棒じゃ。竜骨に手を当てると、木がどれだけの重さに耐えられるか、今でも大体分かる。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Growth_Adult / SourceModule: 竜骨の記憶
- Rarity: 希少
- 技能: 建築+3、工芸+2
- 作業不能: なし / 特性候補: なし
- 組合せ: 同一Eraの幼少期を優先。EraIndependentとの接続時は成人期側の年代を採用。
- 制作注記: 拡張稿（§12 Phase A超過分）。高補正階級=0。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A413 律法学舎の若きソフェル](/backstories/mastery/SHION_A413-律法学舎の若きソフェル.md)
- 同じ出典の次項: [SHION_A415 種Cell研究所の培養手](/backstories/mastery/SHION_A415-種-研究所の培養手.md)
- 正史: [正史](/world/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A414 港湾都市の造船匠`
