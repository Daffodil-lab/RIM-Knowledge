---
type: "Backstory Record"
title: "SHION_A072 身体秘文字医"
description: "人工身体の秘文字配列を治療し、痛覚を消す前に本人がその痛みへ与えた意味を尋ねた。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
eras:
  - "EraIndependent"
organization_groups:
  - "sofer-law"
organization_names:
  - "ソフェル"
  - "造体医療院"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A072"
canonical_scope: "backstory-mastery"
source_section: "SHION_A072 身体秘文字医"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A072 身体秘文字医

## 設定クラス

- Short: 身体秘文字医
- Era: EraIndependent / Origin: ソフェル / Relation: 造体医療院

- Tone: 医療・秘文字 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: NotApplicable
- VoiceProfile: 静かな造体医。痛みの除去を急がず、身体の意味を決める権利を本人へ返す。
- PotentialReferencePoints: 本人は人工身体の秘文字配列を治療し、痛覚を消す前に本人がその痛みへ与えた意味を尋ねた。 / 新しい身体を以前の本人らしさだけで縛らなかった。 / 長い時代が過ぎても、本人はこの出来事を古びた過去として扱わない。

> 人工身体の秘文字配列を治療していました。痛覚異常なら消せばよい、と考えやすいでしょう。けれど患者の中には、その痛みを以前の身体との境界として残したい者もいます。私は処置の前に、何を苦痛と感じ、何を手放したくないか尋ねました。逆に、古い友人から「以前らしい身体へ戻して」と頼まれて苦しむ患者もいた。医師が本人らしさを決め始めたら、診察は中断すべきです。私は身体を直せますが、どの形で生きるべきかまでは処方しません。新しい身体が新しい選択をする余地も、治療の一部です。

**匿名のソフェルの余録：** 同意記録には痛覚の強度だけでなく、本人が残したい感覚と身体像も記載されています。第三者の希望だけで配列を変更した例はありません。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Sofer_Adult / SourceModule: CoreMOD
- Rarity: 専門
- 技能: 医術+10、知力+7、工芸+4
- 作業不能: なし / 特性候補: なし
- 組合せ: 全時代。医療、造体、秘文字系幼少期向け。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A071 帝国祈祷復元者](/backstories/mastery/SHION_A071-帝国祈祷復元者.md)
- 同じ出典の次項: [SHION_A073 星界マソレット](/backstories/mastery/SHION_A073-星界マソレット.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A072 身体秘文字医`
