---
type: "Backstory Record"
title: "SHION_A039 内部改革者"
description: "同盟に残り、配給審査、代表選出、監査公開の仕組みを内側から改めようとした。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "red-star"
eras:
  - "ModernUnion"
organization_groups:
  - "union"
organization_names:
  - "社会民主党系"
  - "同盟内部改革派"
status: stable
authority: catalog
knowledge_role: catalog-record
canon_review: accepted
granularity: record
canonical_for: "backstories/mastery/SHION_A039"
canonical_scope: "backstory-mastery"
source_section: "SHION_A039 内部改革者"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A039 内部改革者

## 設定クラス

- Short: 内部改革者
- Era: ModernUnion / Origin: 社会民主党系 / Relation: 同盟内部改革派

- Tone: 政治・制度改革 / ReviewStatus: 採用済み
- ServiceDomain: NotApplicable
- VoiceProfile: 粘り強く少し疲れた改革者。大改革より、異議申立てに三日を足す実務を選ぶ。
- PreservedCanonPoints: 本人は同盟に残り、配給審査、代表選出、監査公開の仕組みを内側から改めようとした。 / 離脱した同志からは妥協と呼ばれ、執行部からは不忠を疑われた。 / 大きな制度改革は成立しなかったが、異議申立ての期限を延ばし、却下理由を公開させた。

> 同盟に残った私は、離脱した同志から妥協者、執行部から不忠者と呼ばれました。配給審査、代表選出、監査公開を内側から変えようとして、大改革は一つも通せませんでした。通せたのは異議申立てを三日延ばし、却下理由を公開させることです。革命歌にはなりません。でもその三日で書類を揃え、配給を取り戻した住民がいました。蘇った友から「まだ中にいるのか」と呆れられた時は、少し傷つきましたよ。私は制度を愛して残ったのではなく、明日まで待てない人の窓口を今日閉めたくなかったのです。

**匿名のソフェルの余録：** 改正記録は異議申立期間を三日延長し、却下理由の通知を義務化しています。適用後に配給決定が覆った住民記録も複数確認できます。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Political_Adult / SourceModule: Shion Race: Red Star
- Rarity: 専門
- 技能: 社交+7、知力+3
- 作業不能: なし / 特性候補: なし
- 組合せ: ModernUnion。仲裁・五印章・評議会系幼少期向け。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A038 異常対応員](/backstories/mastery/SHION_A038.md)
- 同じ出典の次項: [SHION_A040 外からの社会改良者](/backstories/mastery/SHION_A040.md)
- 正史: [正史](/world/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A039 内部改革者`
