---
type: "Backstory Record"
title: "SHION_A367 世帯調整員の台帳"
description: "世帯調整局で、外部居住者の出生と全住民のパートナー関係・居住区割当を別台帳へ記録した。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "canon"
eras:
  - "JourneyEra"
organization_groups:
  - "other-named"
organization_names:
  - "世帯調整局"
  - "第三世界／香辛料交易文化継承者"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A367"
canonical_scope: "backstory-mastery"
source_section: "SHION_A367 世帯調整員の台帳"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A367 世帯調整員の台帳

## 設定クラス

- Short: 世帯調整台帳
- Era: JourneyEra / Origin: 第三世界／香辛料交易文化継承者 / Relation: 世帯調整局

- Tone: 家族・調整・記憶 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: Civilian
- VoiceProfile: 几帳面な事務屋。項目立てて話す癖が抜けない。
- PotentialReferencePoints: 新規稿のため該当なし。

> 私の仕事は三つの台帳をつけることだった。一つ、外部居住者の出生。二つ、全住民のパートナー関係と世帯。三つ、居住区の割り当て。香辛料交易文化を継ぐ世帯は特に居住区の希望が多くて苦労した。台帳には書けない揉め事もたくさんあった。誰と誰が同じ甲板を嫌がるかとか、そういう細かい話だ。それでも三十年で担当した台帳は数えきれない。今思えば、私が守っていたのは台帳ではなく、その裏にある小さな諍いの記憶だったのかもしれない。ペン先の減り方にまでそれが出ている気がする。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Journey_Adult / SourceModule: 甲板日誌抄
- Rarity: 一般
- 技能: 社交+4、知力+3
- 作業不能: なし / 特性候補: なし
- 組合せ: 同一Eraの幼少期を優先。EraIndependentとの接続時は成人期側の年代を採用。
- 制作注記: 拡張稿（§12 Phase A超過分）。高補正階級=0。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A366 巡回区画の三つの規則](/backstories/mastery/SHION_A366-巡回区画の三つの規則.md)
- 同じ出典の次項: [SHION_A368 家畜甲板の自慢話](/backstories/mastery/SHION_A368-家畜甲板の自慢話.md)
- 正史: [正史](/world/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A367 世帯調整員の台帳`
