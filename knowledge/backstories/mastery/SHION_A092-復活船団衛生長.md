---
type: "Backstory Record"
title: "SHION_A092 復活船団衛生長"
description: "避難船で死者の保存、負傷者の治療、蘇生後の再会を一つの医療計画として扱った。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
eras:
  - "EraIndependent"
organization_groups:
  - "independent-frontier"
organization_names:
  - "星域避難船団"
  - "無党派"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A092"
canonical_scope: "backstory-mastery"
source_section: "SHION_A092 復活船団衛生長"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A092 復活船団衛生長

## 設定クラス

- Short: 復活船団衛生長
- Era: EraIndependent / Origin: 無党派 / Relation: 星域避難船団

- Tone: 医療・避難 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: Military
- VoiceProfile: 包容力ある船団医。救命、死者保存、再会後の沈黙まで一続きの医療として語る。
- PotentialReferencePoints: 本人は避難船で死者の保存、負傷者の治療、蘇生後の再会を一つの医療計画として扱った。 / 生存率に表れない遺族の待ち時間も短くした。 / 長い時代が過ぎても、本人はこの出来事を古びた過去として扱わない。

> 避難船団では、負傷者の治療、死者の保存、蘇生後の再会を別々の仕事にしませんでした。生存率だけ見れば、設備へ収容した時点で成功です。でも家族には、会えるのか、いつなのか、戻った者が自分を覚えているのかという待ち時間が続きます。私は医療計画へ連絡と再会支援を組み込みました。再会しても言葉が出ず、同じ席で黙る家族もいます。その時間も診療です。勤務表は長くなりましたが、到着した瞬間に患者を記録の外へ追い出すより、ずっと正確だと思っています。

**匿名のソフェルの余録：** 船団の医療計画には負傷治療、死者保存、遺族連絡、再会支援が連続した工程として記載されています。到着後の支援記録も残ります。

## 実装予約

- Slot: Adulthood
- Pool: Shion_MilitaryRescue_Adult / SourceModule: CoreMOD
- Rarity: 専門
- 技能: 医術+10、社交+7、知力+4
- 作業不能: なし / 特性候補: なし
- 組合せ: 全時代。医療、避難、蘇生系幼少期向け。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A091 同盟親衛射手](/backstories/mastery/SHION_A091-同盟親衛射手.md)
- 同じ出典の次項: [SHION_A093 帰還船団救命長](/backstories/mastery/SHION_A093-帰還船団救命長.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A092 復活船団衛生長`
