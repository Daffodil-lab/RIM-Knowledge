---
type: "Backstory Record"
title: "SHION_A379 帝国准民の自嘲"
description: "帝国准民なんて肩書きがついた日、俺は道具箱の中身を数えていた。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "canon"
eras:
  - "JourneyEra"
organization_groups:
  - "empire"
organization_names:
  - "金属加工組合"
  - "帝国准民"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A379"
canonical_scope: "backstory-mastery"
source_section: "SHION_A379 帝国准民の自嘲"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A379 帝国准民の自嘲

## 設定クラス

- Short: 帝国准民自嘲
- Era: JourneyEra / Origin: 帝国准民 / Relation: 金属加工組合

- Tone: 自嘲・労働・統合 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: Civilian
- VoiceProfile: 皮肉屋の職人。真面目な話も笑いに包む。
- PotentialReferencePoints: 新規稿のため該当なし。

> 帝国准民なんて肩書きがついた日、俺は道具箱の中身を数えていた。歴史的な瞬間に立ち会った実感なんて全然なくて、ハンマーが一本足りないことの方が気になってた。統合前は組合ごとに規格が違って苦労したが、統合後は規格が一つになって、むしろ仕事は楽になった。偉そうな理由より、ネジの規格が合う方が俺には重要だった。歴史の話をしろと言われても、俺にはネジの話しかできない。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Journey_Adult / SourceModule: 鍛冶場の讃歌
- Rarity: 一般
- 技能: 工芸+4、知力-2
- 作業不能: なし / 特性候補: なし
- 組合せ: 同一Eraの幼少期を優先。EraIndependentとの接続時は成人期側の年代を採用。
- 制作注記: 拡張稿（§12 Phase A超過分）。高補正階級=0。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A378 秩序維持隊の最終巡回](/backstories/mastery/SHION_A378-秩序維持隊の最終巡回.md)
- 同じ出典の次項: [SHION_A380 農区統合の教訓](/backstories/mastery/SHION_A380-農区統合の教訓.md)
- 正史: [正史](/world/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A379 帝国准民の自嘲`
