---
type: "Backstory Record"
title: "SHION_A023 儀式教師"
description: "祈り、葬送、成人式を、信仰や文化の異なる生徒へ教える儀式教師となった。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
eras:
  - "ModernUnion"
organization_groups:
  - "sofer-law"
organization_names:
  - "ソフェル"
  - "共同体学校"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A023"
canonical_scope: "backstory-mastery"
source_section: "SHION_A023 儀式教師"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A023 儀式教師

## 設定クラス

- Short: 儀式教師
- Era: ModernUnion / Origin: ソフェル / Relation: 共同体学校

- Tone: 教育・儀礼 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: NotApplicable
- VoiceProfile: 親しみやすく忍耐強い教師。手順違いを叱るより、なぜ残ったか生徒と調べる。
- PotentialReferencePoints: 本人は祈り、葬送、成人式を、信仰や文化の異なる生徒へ教える儀式教師となった。 / 形だけ学びたい者と意味から知りたい者に、それぞれ別の教材を用意した。 / 手順を間違えた生徒を式から外さず、なぜその順番が残ったか一緒に調べた。

> 祈りの順を間違えた生徒を、式から追い出してはいけません。私は葬送と成人式を、信仰も文化も違う生徒へ教えました。形だけ覚えたい者には短い手順を、意味から知りたい者には帝国式、革命期式、地域式の由来を渡しました。一度、全部を尊重しようとして三つの式を混ぜ、日が暮れても終わらなかったことがあります。生徒には好評でしたが遺族には謝りました。それから違いを消さず、どれを選ぶか本人へ聞きます。儀式は正解を競う試験ではなく、そこにいる者が何を大切にするか確かめる約束です。

**匿名のソフェルの余録：** 教材は帝国式、革命期式、地域式を別系統で示します。三式を混合した長時間の葬送も記録され、以後は主催者による事前選択が加わりました。

## 実装予約

- Slot: Adulthood
- Pool: Sofer_Specialist_Adult / SourceModule: CoreMOD
- Rarity: 一般
- 技能: 社交+6、知力+2、芸術+1
- 作業不能: なし / 特性候補: なし
- 組合せ: ModernUnion。神殿侍祭・無言劇・葬送系幼少期向け。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A022 破門校正官](/backstories/mastery/SHION_A022.md)
- 同じ出典の次項: [SHION_A024 身体文章設計者](/backstories/mastery/SHION_A024.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A023 儀式教師`
