---
type: "Backstory Record"
title: "SHION_A369 律法以前の祈り手"
description: "律法以前の祈り手では、まだ名前もなかった、本人たちがしていたことには。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "canon"
eras:
  - "JourneyEra"
organization_groups:
  - "other-named"
organization_names:
  - "初期儀礼班"
  - "信徒見習い"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A369"
canonical_scope: "backstory-mastery"
source_section: "SHION_A369 律法以前の祈り手"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A369 律法以前の祈り手

## 設定クラス

- Short: 律法以前祈り手
- Era: JourneyEra / Origin: 信徒見習い / Relation: 初期儀礼班

- Tone: 信仰・形成・儀礼 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: NotApplicable
- VoiceProfile: 落ち着いた声で同じ言葉を繰り返す語り部。
- PotentialReferencePoints: 新規稿のため該当なし。

> まだ名前もなかった、私たちがしていたことには。朝に光を確認する、それだけの儀礼だった。朝に光を確認する。それを十年続けたら、隣の甲板の人も真似し始めた。朝に光を確認する。誰かがそれに言葉をつけ、律法と呼ぶようになったのは、私が儀礼を始めてからずっと後のことだ。朝に光を確認する。今も私はそれだけをしている。名前がついても、していることは変わらない。窓に手を当てる角度さえ、最初の日から変えていない。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Journey_Adult / SourceModule: 律法の産声
- Rarity: 希少専門
- 技能: 社交+3、知力+2
- 作業不能: なし / 特性候補: なし
- 組合せ: 同一Eraの幼少期を優先。EraIndependentとの接続時は成人期側の年代を採用。
- 制作注記: 拡張稿（§12 Phase A超過分）。高補正階級=0。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A368 家畜甲板の自慢話](/backstories/mastery/SHION_A368.md)
- 同じ出典の次項: [SHION_A370 共同礼拝堂の梁](/backstories/mastery/SHION_A370.md)
- 正史: [正史](/world/index.md)
- 制作・開示規則: [制作・開示規則](/authoring/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A369 律法以前の祈り手`
