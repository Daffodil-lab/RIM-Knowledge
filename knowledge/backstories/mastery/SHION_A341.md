---
type: "Backstory Record"
title: "SHION_A341 旧内陸連合圏の密造酒職人"
description: "旧内陸連合圏の密造酒職人では、船内で酒を造るのは規則違反だった。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "canon"
eras:
  - "JourneyEra"
organization_groups:
  - "other-named"
organization_names:
  - "居住区自治会"
  - "西側／作られた民族"
status: stable
authority: catalog
knowledge_role: catalog-record
canon_review: accepted
granularity: record
canonical_for: "backstories/mastery/SHION_A341"
canonical_scope: "backstory-mastery"
source_section: "SHION_A341 旧内陸連合圏の密造酒職人"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A341 旧内陸連合圏の密造酒職人

## 設定クラス

- Short: 密造酒職人
- Era: JourneyEra / Origin: 西側／作られた民族 / Relation: 居住区自治会

- Tone: 密造・規則・自嘲 / ReviewStatus: 採用済み
- ServiceDomain: Civilian
- VoiceProfile: 皮肉屋の職人。深刻な話ほど笑いに変える。
- PreservedCanonPoints: 新規稿のため該当なし。

> 船内で酒を造るのは規則違反だった。俺は二十年それを破り続けた立派な犯罪者だったわけだ。作られた民族文化を継いだ醸造の先達のレシピを頼りに、余った穀物の皮で発酵させた酒は、正直言ってひどい味だった。それでも自治会の連中は文句を言いながら毎晩飲みに来た。摘発されたのは三回、罰は毎回同じ量の酒を没収されるだけだった。つまり規則は俺の在庫管理を手伝ってくれていたわけだ。褒められた話じゃないが、悪くもなかったと思ってる。

**匿名のソフェルの聞き取り：** 「反省は？」 — 「まったくない」

## 実装予約

- Slot: Adulthood
- Pool: Shion_Journey_Adult / SourceModule: 継ぎ接ぎの故郷
- Rarity: 一般
- 技能: 料理+4、社交+3
- 作業不能: なし / 特性候補: なし
- 組合せ: 同一Eraの幼少期を優先。EraIndependentとの接続時は成人期側の年代を採用。
- 制作注記: 拡張稿（§12 Phase A超過分）。高補正階級=0。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A340 航行記録班の観測員](/backstories/mastery/SHION_A340.md)
- 同じ出典の次項: [SHION_A342 医療分隊の助産師](/backstories/mastery/SHION_A342.md)
- 正史: [正史](/world/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A341 旧内陸連合圏の密造酒職人`
