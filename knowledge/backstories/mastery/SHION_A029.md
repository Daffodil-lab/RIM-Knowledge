---
type: "Backstory Record"
title: "SHION_A029 遺物回収屋"
description: "廃墟から工具、医薬品、記録媒体、再利用できる建材を回収した。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
eras:
  - "EraIndependent"
organization_groups:
  - "independent-frontier"
organization_names:
  - "独立回収団"
  - "無党派"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A029"
canonical_scope: "backstory-mastery"
source_section: "SHION_A029 遺物回収屋"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A029 遺物回収屋

## 設定クラス

- Short: 遺物回収屋
- Era: EraIndependent / Origin: 無党派 / Relation: 独立回収団

- Tone: 探索・危険作業 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: NotApplicable
- VoiceProfile: 胆力のある現実家。市場価値より当日の用途を見て、盗掘と救助の境界に敏感である。
- PotentialReferencePoints: 本人は廃墟から工具、医薬品、記録媒体、再利用できる建材を回収した。 / 市場価格より、持ち帰った日に開拓地で使えるかを基準に優先順位を決めた。 / 帝国の印章がある品は発見場所と時代を記録し、最盛期の公共施設と内戦後の拘禁施設から出た物を同じ由来として売らなかった。

> 廃墟で薬を持ち帰れば救助、祭具を売れば盗掘と呼ばれます。境界は鞄でなく、中身と持ち主の許可にあります。私は工具、医薬品、記録媒体、建材を、市場価格よりその日に使えるかで選びました。帝国印の品は発見場所と年代を残し、最盛期の公共施設と内戦後の拘禁室を同じ由来では売りません。一度、急いで遺族への確認を省き、思い出の器具を資材にしてしまいました。時間なら十分にあったのに。今は持ち帰れるかより、誰へ尋ね終えていないかを先に調べます。

**匿名のソフェルの余録：** 回収台帳は用途、発見場所、年代、許可者を記録しています。無断転用した器具は遺族へ返還され、以後は照会完了前の解体が禁じられました。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Frontier_Adult / SourceModule: CoreMOD
- Rarity: 専門
- 技能: 射撃+4、採掘+3、工芸+2
- 作業不能: なし / 特性候補: なし
- 組合せ: EraIndependent。鎧名読み・遺物番・二つ目の影系幼少期向け。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A028 辺境農芸師](/backstories/mastery/SHION_A028.md)
- 同じ出典の次項: [SHION_A030 救難記録編纂者](/backstories/mastery/SHION_A030.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A029 遺物回収屋`
