---
type: "Backstory Record"
title: "SHION_A372 採掘班の沈黙"
description: "採掘班の沈黙では、その年のことは、あまり話すことがない。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "canon"
eras:
  - "JourneyEra"
organization_groups:
  - "other-named"
organization_names:
  - "儀礼班"
  - "資源採掘班"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A372"
canonical_scope: "backstory-mastery"
source_section: "SHION_A372 採掘班の沈黙"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A372 採掘班の沈黙

## 設定クラス

- Short: 採掘班沈黙
- Era: JourneyEra / Origin: 儀礼班 / Relation: 資源採掘班

- Tone: 労働・沈黙・喪失 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: Civilian
- VoiceProfile: 言葉少なな採掘工。核心には触れず周辺だけ語る。
- PotentialReferencePoints: 新規稿のため該当なし。

> あの年のことは、あまり話すことがない。採掘区画の圧が変わって、班の配置が急に組み替えられた、それだけだ。自分は儀礼班にも少し関わっていたから、変わった配置の中で祈りの時間だけは守った。工具の手入れは相棒から一人で覚えた。詳しいことは、もう自分の口からは言わない。ただ、あの区画の壁の色だけは今でも覚えている、灰色よりもう少し青かった。それだけ聞いてもらえれば十分だ。

**匿名のソフェルの聞き取り：** 「あの年に何があったのですか」 — 「壁の色の話しかしません」

## 実装予約

- Slot: Adulthood
- Pool: Shion_Journey_Adult / SourceModule: 律法以前
- Rarity: 一般
- 技能: 採掘+4、医術-1
- 作業不能: なし / 特性候補: なし
- 組合せ: 同一Eraの幼少期を優先。EraIndependentとの接続時は成人期側の年代を採用。
- 制作注記: 拡張稿（§12 Phase A超過分）。高補正階級=0。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A371 混成学舎の初年度](/backstories/mastery/SHION_A371-混成学舎の初年度.md)
- 同じ出典の次項: [SHION_A373 混成世代の勘違い集](/backstories/mastery/SHION_A373-混成世代の勘違い集.md)
- 正史: [正史](/world/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A372 採掘班の沈黙`
