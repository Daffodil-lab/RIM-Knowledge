---
type: "Backstory Record"
title: "SHION_A093 帰還船団救命長"
description: "時間嵐で分断された帰還船団を追い、蘇生設備へ届く前の救命で多数を帰した。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
eras:
  - "EraIndependent"
organization_groups:
  - "empire"
  - "independent-frontier"
organization_names:
  - "星域帰還船団"
  - "帝国離反者系"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A093"
canonical_scope: "backstory-mastery"
source_section: "SHION_A093 帰還船団救命長"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A093 帰還船団救命長

## 設定クラス

- Short: 帰還船団救命長
- Era: EraIndependent / Origin: 帝国離反者系 / Relation: 星域帰還船団

- Tone: 医療・救命 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: Military
- VoiceProfile: 執念深い救命長。表彰を受けながらも、時間嵐へ消えた一隻の捜索をやめない。
- PotentialReferencePoints: 本人は時間嵐で分断された帰還船団を追い、蘇生設備へ届く前の救命で多数を帰した。 / 同族から勲章級と評されても、間に合わなかった一隻の航路を探し続けた。 / 新しい身体と職を得た後も、本人はそこで抱いた気持ちを手放さなかった。

> 時間嵐で分断された帰還船団を追い、蘇生設備へ着く前に救命を続けました。多くの船を連れ戻し、救命章も受けましたよ。それでも一隻だけ、航跡が嵐の中で途切れたままです。表彰式の日も捜索窓を閉じられず、写真では横を向いています。無礼だったとは思います。でも、見つからないことと存在しないことは同じではありません。時間の外へ流された可能性が一つでも残るなら、帰還路を更新し続けます。章は終わった任務へ与えられましたが、私にとってあの船団の任務はまだ終わっていないんです。

**匿名のソフェルの余録：** 星域救命章と船団救助記録を確認しました。未帰還船一隻の捜索案件は現在も閉じられず、新しい航路情報が届くたび再照合されています。

## 実装予約

- Slot: Adulthood
- Pool: Shion_MilitaryRescue_Adult / SourceModule: CoreMOD
- Rarity: 叙勲級
- 技能: 医術+12、知力+8、射撃+4
- 作業不能: なし / 特性候補: なし
- 組合せ: 全時代。避難列車、通信、蘇生系幼少期向け。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A092 復活船団衛生長](/backstories/mastery/SHION_A092-復活船団衛生長.md)
- 同じ出典の次項: [SHION_A094 帝国平和復元派](/backstories/mastery/SHION_A094-帝国平和復元派.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A093 帰還船団救命長`
