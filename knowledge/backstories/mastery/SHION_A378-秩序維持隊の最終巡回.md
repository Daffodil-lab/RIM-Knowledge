---
type: "Backstory Record"
title: "SHION_A378 秩序維持隊の最終巡回"
description: "秩序維持隊の最終巡回では、秩序維持隊の任務は単純だった。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "canon"
eras:
  - "JourneyEra"
organization_groups:
  - "empire"
organization_names:
  - "秩序維持隊"
  - "帝国准民"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A378"
canonical_scope: "backstory-mastery"
source_section: "SHION_A378 秩序維持隊の最終巡回"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A378 秩序維持隊の最終巡回

## 設定クラス

- Short: 秩序維持最終
- Era: JourneyEra / Origin: 帝国准民 / Relation: 秩序維持隊

- Tone: 秩序・軍務・統合 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: Military
- VoiceProfile: 感情を抑えた元隊員。手順のように過去を語る。
- PotentialReferencePoints: 新規稿のため該当なし。

> 秩序維持隊の任務は単純だった。区画境界の見回り、揉め事の仲裁、非常時の避難誘導。旧い保安巡回とは違って、担当区画に出身圏の区別はなかった。俺が配属された年にはもう帝国という呼び方が広まっていて、隊の徽章もそれに合わせて変わった、重さも色も変わらないのに不思議と誇らしかった。任務の内容は変わらなかったが、呼び方が変わると士気は不思議と上がった。最後の巡回では、境界線そのものがほとんど意味を失っていた。

**匿名のソフェルの聞き取り：** 「境界が意味を失うとは」 — 「見回る理由が薄れたということです」

## 実装予約

- Slot: Adulthood
- Pool: Shion_Journey_Adult / SourceModule: 名もなき航路
- Rarity: 希少
- 技能: 格闘+3、社交+3
- 作業不能: なし / 特性候補: なし
- 組合せ: 同一Eraの幼少期を優先。EraIndependentとの接続時は成人期側の年代を採用。
- 制作注記: 拡張稿（§12 Phase A超過分）。高補正階級=0。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A377 統合行政区の初日](/backstories/mastery/SHION_A377-統合行政区の初日.md)
- 同じ出典の次項: [SHION_A379 帝国准民の自嘲](/backstories/mastery/SHION_A379-帝国准民の自嘲.md)
- 正史: [正史](/world/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A378 秩序維持隊の最終巡回`
