---
type: "Backstory Record"
title: "SHION_A338 居住区の家庭教師"
description: "居住区で大河の民の言語と子守唄を教え、使う機会の少ない文化記憶を少数の継承者へ渡した。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "canon"
eras:
  - "JourneyEra"
organization_groups:
  - "other-named"
organization_names:
  - "教育共同体"
  - "東側／大河の民"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A338"
canonical_scope: "backstory-mastery"
source_section: "SHION_A338 居住区の家庭教師"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A338 居住区の家庭教師

## 設定クラス

- Short: 居住区家庭教師
- Era: JourneyEra / Origin: 東側／大河の民 / Relation: 教育共同体

- Tone: 教育・世代・言語 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: Civilian
- VoiceProfile: 穏やかな家庭教師。数字にこだわって語る。
- PotentialReferencePoints: 新規稿のため該当なし。

> 僕が担当した生徒は生涯で二百八人だった。いや、二百九人だ、最後の一人を数え忘れていた。大河の民の言葉を教えられる教師はもう船内に僕しかいなくなっていて、それでも生徒の大半はその言葉を使う機会もなく育った。三人だけ、故郷の子守唄を最後まで覚えてくれた。多いとは言えない数字だけど、僕はその三人のために残りの生涯を使った。二百九人のうち三人、それで十分だと思っている。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Journey_Adult / SourceModule: 継承者たちの朝
- Rarity: 希少
- 技能: 知力+5、社交+2
- 作業不能: なし / 特性候補: なし
- 組合せ: 同一Eraの幼少期を優先。EraIndependentとの接続時は成人期側の年代を採用。
- 制作注記: 拡張稿（§12 Phase A超過分）。高補正階級=0。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A337 甲板食堂の調理人](/backstories/mastery/SHION_A337-甲板食堂の調理人.md)
- 同じ出典の次項: [SHION_A339 甲板建築班の左官](/backstories/mastery/SHION_A339-甲板建築班の左官.md)
- 正史: [正史](/world/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A338 居住区の家庭教師`
