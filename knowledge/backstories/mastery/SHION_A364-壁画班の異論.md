---
type: "Backstory Record"
title: "SHION_A364 壁画班の異論"
description: "故郷の風景を描いて心を慰めた、とよく言われるけれど違う。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "canon"
eras:
  - "JourneyEra"
organization_groups:
  - "other-named"
organization_names:
  - "共用区画壁画班"
  - "東側／東方の民"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A364"
canonical_scope: "backstory-mastery"
source_section: "SHION_A364 壁画班の異論"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A364 壁画班の異論

## 設定クラス

- Short: 壁画班の異論
- Era: JourneyEra / Origin: 東側／東方の民 / Relation: 共用区画壁画班

- Tone: 芸術・記憶・異議 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: NotApplicable
- VoiceProfile: 静かに反論する画家。決めつけを嫌う。
- PotentialReferencePoints: 新規稿のため該当なし。

> 故郷の風景を描いて心を慰めた、とよく言われるけれど違う。私が壁に描いたのは見たことのない景色ばかりだった。東方の民としての記憶は前世代の記録でしか知らず、私自身は船の中でしか生きていない。だから描いたのは想像の丘、想像の川、見たこともない雨の匂い。それを望郷画と呼ばれるたび、少し訂正したくなる。私が描いていたのは失った故郷ではなく、まだ見ぬどこかへの当てずっぽうだったのだと、絵の前でいつも言い直している。

**匿名のソフェルの聞き取り：** 「望郷の絵ではないと」 — 「望郷する対象を、私は持っていません」

## 実装予約

- Slot: Adulthood
- Pool: Shion_Journey_Adult / SourceModule: 星屑の継承図
- Rarity: 希少
- 技能: 芸術+5、知力+2
- 作業不能: なし / 特性候補: なし
- 組合せ: 同一Eraの幼少期を優先。EraIndependentとの接続時は成人期側の年代を採用。
- 制作注記: 拡張稿（§12 Phase A超過分）。高補正階級=0。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A363 水耕甲板の霜焼け](/backstories/mastery/SHION_A363-水耕甲板の低温損傷.md)
- 同じ出典の次項: [SHION_A365 隔離区画の手順書](/backstories/mastery/SHION_A365-隔離区画の手順書.md)
- 正史: [正史](/world/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A364 壁画班の異論`
