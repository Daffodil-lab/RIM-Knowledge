---
type: "Backstory Record"
title: "SHION_A085 独立星港建設者"
description: "社会民主党員、旧共産党員、ファシスト離党者、帝国離反者、無党派が使う星港を建てた。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "independent-colony"
  - "equipment"
eras:
  - "ModernUnion"
organization_groups:
  - "independent-frontier"
  - "union"
organization_names:
  - "混成開拓評議会"
  - "独立開拓団"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A085"
canonical_scope: "backstory-mastery"
source_section: "SHION_A085 独立星港建設者"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A085 独立星港建設者

## 設定クラス

- Short: 独立星港建設者
- Era: ModernUnion / Origin: 独立開拓団 / Relation: 混成開拓評議会

- Tone: 建築・共同体 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: NotApplicable
- VoiceProfile: 現実的な建設者。思想の違いを認めつつ、空気漏れと武器庫には共通規則を求める。
- PotentialReferencePoints: 本人は社会民主党員、旧共産党員、ファシスト離党者、帝国離反者、無党派が使う星港を建てた。 / 入口に思想審査を置かず、武器庫だけ全員同じ規則で閉じた。 / 新しい身体と職を得た後も、本人はそこで抱いた気持ちを手放さなかった。

> 社会民主党員、旧共産党員、ファシスト離党者、帝国離反者、無党派。皆で使う独立星港を建てました。会議では何度も揉めましたよ。共有倉庫の所有権から壁の色まで一致しない。だから入口に思想審査は置かず、武器庫と気密作業だけ全員同じ規則にしました。空気漏れは党籍を見て止まってくれませんからね。完成後も議論は続いています。むしろ健全でしょう。共同体は同じ考えの者だけで作る物ではなく、違うまま同じ港を直せる者が作る。私はそう思って図面を引きました。

**匿名のソフェルの余録：** 星港評議会の所属構成は供述どおりです。利用規約に思想審査はなく、武器庫管理と気密事故対応だけは全団体の共通手順になっています。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Frontier_Adult / SourceModule: CoreMOD
- Rarity: 専門
- 技能: 建築+10、工芸+6、社交+3、採掘+1
- 作業不能: なし / 特性候補: なし
- 組合せ: ModernUnion。建築、混住学校、党派周辺系幼少期向け。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A084 多重勲章の親衛兵](/backstories/mastery/SHION_A084-多重勲章の親衛兵.md)
- 同じ出典の次項: [SHION_A086 故郷種子の運び手](/backstories/mastery/SHION_A086-故郷種子の運び手.md)
- 独立開拓団: [独立開拓団](/colony/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A085 独立星港建設者`
