---
type: "Backstory Record"
title: "SHION_A322 育種班の選種人"
description: "育種班の選種人では、試した品種は三十七、いや正確には三十八だった。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "canon"
eras:
  - "ColonizationEra"
organization_groups:
  - "independent-frontier"
organization_names:
  - "育種班"
  - "第一次入植"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A322"
canonical_scope: "backstory-mastery"
source_section: "SHION_A322 育種班の選種人"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A322 育種班の選種人

## 設定クラス

- Short: 選種人
- Era: ColonizationEra / Origin: 育種班 / Relation: 第一次入植

- Tone: 育種・選別・継承 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: Civilian
- VoiceProfile: ある数字にこだわり、何度も言い直しながら語る選種人。
- PotentialReferencePoints: 新規稿のため該当なし。

> 試した品種は三十七、いや正確には三十八だった。一つは記録に残す前に霜でだめになったから、数えるか迷っていた。育種班では第一次入植の作物を少しずつ土地に合わせて選び直す仕事をしていて、三十八のうち根付いたのは六つだけだ。少ないと思うかもしれないが、最初の年は一つも根付かなかったことを思えば十分な数字だ。今、畑に並んでいるのはその六つの子孫にあたる。三十八という数字を覚えているのは、たぶん私だけだろう。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Colonization_Adult / SourceModule: 種の余白
- Rarity: 希少
- 技能: 栽培+4、動物+2
- 作業不能: なし / 特性候補: なし
- 組合せ: 同一Eraの幼少期を優先。EraIndependentとの接続時は成人期側の年代を採用。
- 制作注記: 拡張稿（§12 Phase A超過分）。高補正階級=0。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A321 境界哨戒隊の見張り番](/backstories/mastery/SHION_A321-境界哨戒隊の見張り番.md)
- 同じ出典の次項: [SHION_A323 気象観測班の雲読み](/backstories/mastery/SHION_A323-気象観測班の雲読み.md)
- 正史: [正史](/world/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A322 育種班の選種人`
