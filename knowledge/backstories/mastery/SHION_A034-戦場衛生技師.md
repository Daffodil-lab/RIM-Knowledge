---
type: "Backstory Record"
title: "SHION_A034 戦場衛生技師"
description: "革命連合の衛生隊で、外部居住者の銃創、熱傷、感染と、シオンの外装・機能部損傷を所属に関係なく処置した。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
eras:
  - "RevolutionaryWar"
organization_groups:
  - "revolutionary-coalition"
organization_names:
  - "革命連合"
  - "混成衛生隊"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A034"
canonical_scope: "backstory-mastery"
source_section: "SHION_A034 戦場衛生技師"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A034 戦場衛生技師

## 設定クラス

- Short: 戦場衛生技師
- Era: RevolutionaryWar / Origin: 革命連合 / Relation: 混成衛生隊

- Tone: 戦場医療・救難 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: Military
- VoiceProfile: 豪胆で公平な衛生技師。党章を外し、傷の深さだけで治療順を決める。
- PotentialReferencePoints: 本人は革命連合の衛生隊で、外部居住者の銃創、熱傷、感染と、シオンの外装・機能部損傷を所属に関係なく処置した。 / 担架へ載せた患者の党章を外し、傷の深さだけで治療順を決めた。 / 設備が不足する現場では弾薬箱や車両を手術台へ変え、異なる党派出身の衛生兵と共通手順を作った。

> 担架へ載せたら党章を外しました。社会民主党でも共産党でも帝国離反者でも、治療順は傷の深さで決めます。革命連合の衛生隊で、私は外部居住者の銃創、熱傷、感染と、シオンの外装・機能部損傷を処置しました。弾薬箱を手術台にし、党派ごとに違った衛生手順を一つずつ合わせました。敵側の負傷者を先に診て仲間から殴られたこともあります。私は殴り返さず、次の患者を運ばせました。戦争が終わって役目が変わっても、党章より外部居住者の出血とシオンの損傷状態を見る癖は捨てません。守る対象を選ぶ前に、まず誰が死にかけているかを確かめます。

**匿名のソフェルの余録：** 混成衛生隊の共通手順は所属欄と治療順位を分離しています。敵側負傷者の先行処置と、その後の隊内暴力も医療日誌へ記載されました。

## 実装予約

- Slot: Adulthood
- Pool: Shion_WarSurvivor_Adult / SourceModule: カタロニア讃歌
- Rarity: 専門
- 技能: 医術+7、射撃+2、社交+1
- 作業不能: なし / 特性候補: なし
- 組合せ: RevolutionaryWar。内戦記録・連合伝令・避難書庫系幼少期向け。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A033 鉄塊護衛](/backstories/mastery/SHION_A033-鉄塊護衛.md)
- 同じ出典の次項: [SHION_A035 防塞工兵](/backstories/mastery/SHION_A035-防塞工兵.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A034 戦場衛生技師`
