---
type: "Backstory Record"
title: "SHION_A087 辺境共同体史家"
description: "党派の指導者より、井戸を掘った者、最初に冬を越した動物、喧嘩の後も残った料理を記録した。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "independent-colony"
eras:
  - "ModernUnion"
organization_groups:
  - "independent-frontier"
organization_names:
  - "開拓地記録会"
  - "独立開拓団"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A087"
canonical_scope: "backstory-mastery"
source_section: "SHION_A087 辺境共同体史家"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A087 辺境共同体史家

## 設定クラス

- Short: 辺境共同体史家
- Era: ModernUnion / Origin: 独立開拓団 / Relation: 開拓地記録会

- Tone: 記録・政治周辺 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: NotApplicable
- VoiceProfile: 庶民的な史家。指導者の演説より、井戸・動物・鍋が共同体を支えた経緯を楽しげに語る。
- PotentialReferencePoints: 本人は党派の指導者より、井戸を掘った者、最初に冬を越した動物、喧嘩の後も残った料理を記録した。 / 小さな共同体の平和も歴史だと考えた。 / 本人は現在も、当時に覚えた手順と名前をどちらも大切にしている。

> 辺境の年代記を書きました。党派指導者の演説も載せましたが、それより井戸を掘った者、最初の冬を越した荷獣、喧嘩の翌日にも皆へ出された煮込み料理を長く記しました。英雄史ではないと批判されましたよ。でも開拓地が残った理由は、演説より凍結した管を直した手にあることが多い。小さな共同体で、昨日争った者が同じ鍋から食べられた一日も平和の歴史です。私は大事件の間にある日常を空白にしたくありません。そこにこそ、皆が守りたかった暮らしが見えるからです。

**匿名のソフェルの余録：** 年代記は指導者名だけでなく、井戸の保守者、越冬した動物、共同食の献立まで収録しています。生活史資料として他の開拓地でも参照されました。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Frontier_Adult / SourceModule: CoreMOD
- Rarity: 一般
- 技能: 知力+7、芸術+4、採掘−2
- 作業不能: なし / 特性候補: なし
- 組合せ: ModernUnion。開拓、記録、料理系幼少期向け。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A086 故郷種子の運び手](/backstories/mastery/SHION_A086-故郷種子の運び手.md)
- 同じ出典の次項: [SHION_A088 蘇生設備の巡回工](/backstories/mastery/SHION_A088-蘇生設備の巡回工.md)
- 独立開拓団: [独立開拓団](/colony/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A087 辺境共同体史家`
