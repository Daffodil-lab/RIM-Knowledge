---
type: "Backstory Record"
title: "SHION_A025 独立開拓者"
description: "行政圏外の谷で、出身の異なる移住者と住居、畑、発電設備を築いた。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "independent-colony"
eras:
  - "EraIndependent"
organization_groups:
  - "independent-frontier"
organization_names:
  - "独立開拓団"
  - "無党派"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A025"
canonical_scope: "backstory-mastery"
source_section: "SHION_A025 独立開拓者"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A025 独立開拓者

## 設定クラス

- Short: 独立開拓者
- Era: EraIndependent / Origin: 無党派 / Relation: 独立開拓団

- Tone: 開拓・自活 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: NotApplicable
- VoiceProfile: 実直で大らかな開拓者。思想では加入者を選ばず、夜番と分配の約束には厳しい。
- PotentialReferencePoints: 本人は行政圏外の谷で、出身の異なる移住者と住居、畑、発電設備を築いた。 / 政治から離れたい者、仕事を求める者、旧党派へ属した者も同じ開拓団に加わった。 / 加入条件を思想で決めず、夜番、清掃、物資分配の規則だけは全員に等しく適用した。

> 行政圏外の谷へ来た理由は、誰にも同じではありません。政治から逃げた者、仕事を探す者、社会民主党や共産党にいた者、ファシスト党を去った者、帝国離反者もいました。私は思想を加入条件にせず、住居と畑と発電設備を一緒に築きました。ただし夜番、清掃、物資分配は全員同じです。立派な主張で夜番を免れようとした者には、私もかなり厳しくしました。一度、古い党歴だけで隣人を疑い、畑の火事への対応を遅らせたことがあります。それから経歴より、今どちらへ水を運ぶかを見るようにしています。

**匿名のソフェルの余録：** 開拓団名簿には複数の旧党派出身者と無党派が並びます。加入規則は思想を問わず、夜番、清掃、分配の義務を共通条件としました。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Frontier_Adult / SourceModule: CoreMOD
- Rarity: 一般
- 技能: 建築+3、栽培+3、射撃+2、動物+1
- 作業不能: なし / 特性候補: なし
- 組合せ: EraIndependent。日常職・渡り徒弟・旅行系幼少期と広く接続。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A024 身体文章設計者](/backstories/mastery/SHION_A024-身体文章設計者.md)
- 同じ出典の次項: [SHION_A026 キャラバン修理師](/backstories/mastery/SHION_A026-キャラバン修理師.md)
- 独立開拓団: [独立開拓団](/colony/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A025 独立開拓者`
