---
type: "Backstory Record"
title: "SHION_A056 故郷模型都市家"
description: "帝国最盛期の故郷を都市模型へ再現したが、宮殿より友人と迷った路地へ多くの時間を使った。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
eras:
  - "EraIndependent"
organization_groups:
  - "empire"
organization_names:
  - "個人工房"
  - "帝国系移民"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A056"
canonical_scope: "backstory-mastery"
source_section: "SHION_A056 故郷模型都市家"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A056 故郷模型都市家

## 設定クラス

- Short: 故郷模型都市家
- Era: EraIndependent / Origin: 帝国系移民 / Relation: 個人工房

- Tone: 建築・郷愁 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: NotApplicable
- VoiceProfile: 懐旧的な模型作家。精密さを誇りながら、存在しなかった公園には照れた希望を込める。
- PotentialReferencePoints: 本人は帝国最盛期の故郷を都市模型へ再現したが、宮殿より友人と迷った路地へ多くの時間を使った。 / 完成後も、当時なかった公園を一つ足した。 / 長い時代が過ぎても、本人はこの出来事を古びた過去として扱わない。

> 帝国最盛期の故郷を、卓上都市として作りました。宮殿の柱は記録どおりですが、いちばん時間を使ったのは、友人と道に迷った細い路地です。角の店、欠けた標識、二人で雨宿りした庇まで覚えていました。完成後、史実にはない公園を一つ加えたんです。復元模型としては反則でしょうね。でも、あの街に戻れるなら、今度は迷った先で休める場所がほしかった。私は過去を同じ形で閉じ込めたいわけではありません。懐かしい暮らしを土台に、昔より少し優しい街を作りたいんです。

**匿名のソフェルの余録：** 宮殿と街路は保存測量に沿っていますが、公園だけは旧地図に存在しません。展示札では復元箇所と本人の提案が明確に分けられています。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Culture_Adult / SourceModule: CoreMOD
- Rarity: 一般
- 技能: 建築+7、芸術+2、知力+1
- 作業不能: なし / 特性候補: なし
- 組合せ: 全時代。建築、帝国最盛期、芸術系幼少期向け。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A055 動物終生保護官](/backstories/mastery/SHION_A055-動物終生保護官.md)
- 同じ出典の次項: [SHION_A057 祝祭調律師](/backstories/mastery/SHION_A057-祝祭調律師.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A056 故郷模型都市家`
