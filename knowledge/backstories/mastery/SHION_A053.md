---
type: "Backstory Record"
title: "SHION_A053 時間差郵便局長"
description: "発送前に届く手紙、差出人の蘇生後に戻る小包、消えた時刻からの葉書を仕分けた。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
eras:
  - "EraIndependent"
organization_groups:
  - "other-named"
organization_names:
  - "星域郵便網"
  - "無党派"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A053"
canonical_scope: "backstory-mastery"
source_section: "SHION_A053 時間差郵便局長"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A053 時間差郵便局長

## 設定クラス

- Short: 時間差郵便局長
- Era: EraIndependent / Origin: 無党派 / Relation: 星域郵便網

- Tone: 通信・時間 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: NotApplicable
- VoiceProfile: 几帳面だが情に厚い局長。時間の矛盾を郵便実務として扱い、乾いた冗談を挟む。
- PotentialReferencePoints: 本人は発送前に届く手紙、差出人の蘇生後に戻る小包、消えた時刻からの葉書を仕分けた。 / 配達不能でも捨てず、宛先が再び存在する日を待った。 / 新しい身体と職を得た後も、本人はそこで抱いた気持ちを手放さなかった。

> 発送される三日前に届いた手紙も、差出人が蘇生した後で戻ってきた小包も扱いました。消えた時刻から葉書が来ることさえあります。郵便局では、因果より先に宛名を確認するんです。配達できない物も捨てません。住所が壊れたのか、受取人がまだ戻っていないのか、それともまだ稼働・登録されていないのかで棚を分けました。保管期限を問われるたび「受取人が存在するまで」と答えたら、倉庫係には嫌われましたね。それでも一通の恋文が数時代越しに届いた日、彼も黙って棚を増やしてくれました。

**匿名のソフェルの余録：** 郵便台帳には発送日より早い受領印が複数あります。局長が設けた存在保留棚から、受取人の蘇生後に配達された手紙も確認されました。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Culture_Adult / SourceModule: CoreMOD
- Rarity: 専門
- 技能: 知力+6、社交+3、工芸+1
- 作業不能: なし / 特性候補: なし
- 組合せ: 全時代。通信、時間技術、旅行系幼少期向け。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A052 遺族連絡官](/backstories/mastery/SHION_A052.md)
- 同じ出典の次項: [SHION_A054 古服再生師](/backstories/mastery/SHION_A054.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A053 時間差郵便局長`
