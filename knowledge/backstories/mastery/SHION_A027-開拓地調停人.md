---
type: "Backstory Record"
title: "SHION_A027 開拓地調停人"
description: "開拓地で土地、水、配給、旧党派の旗を巡る争いを仲裁した。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "independent-colony"
eras:
  - "EraIndependent"
organization_groups:
  - "independent-frontier"
organization_names:
  - "旧党派混成"
  - "独立開拓団"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A027"
canonical_scope: "backstory-mastery"
source_section: "SHION_A027 開拓地調停人"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A027 開拓地調停人

## 設定クラス

- Short: 開拓地調停人
- Era: EraIndependent / Origin: 旧党派混成 / Relation: 独立開拓団

- Tone: 共同体・調停 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: NotApplicable
- VoiceProfile: 世話焼きで現実的。党歴の議論が止まると、井戸掘りや屋根直しへ全員を連れ出す。
- PotentialReferencePoints: 本人は開拓地で土地、水、配給、旧党派の旗を巡る争いを仲裁した。 / 出身党派は確認しても、それを現在の忠誠や要求と同じものとして扱わなかった。 / 話し合いが止まると、屋根の修理や井戸掘りなど双方が必要とする共同作業を提案した。

> 開拓地の争いは土地、水、配給、そして古い旗です。私は党歴を聞きますが、今の忠誠と同じだとは決めません。話し合いが止まれば、双方を井戸掘りか屋根直しへ連れ出しました。喉が渇けば、水の分け方は少し具体的になります。一度、仲直りしたように見えた二人へ昔の旗を一緒に畳ませ、かえって関係を壊しました。再会や和解を儀式で急がせてはいけないんです。旗を捨てても井戸当番は残ります。私は大きな合意より、翌朝に同じ滑車を握れる約束から共同体を作りました。

**匿名のソフェルの余録：** 仲裁記録は出身党派と現在の要求を別欄にしています。共同作業後の合意も恒久的和解とは記さず、井戸と屋根の当番だけを明文化しました。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Frontier_Adult / SourceModule: CoreMOD
- Rarity: 専門
- 技能: 社交+7、建築+1
- 作業不能: なし / 特性候補: なし
- 組合せ: EraIndependent。仲裁・五印章・渡り徒弟系幼少期向け。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A026 キャラバン修理師](/backstories/mastery/SHION_A026-キャラバン修理師.md)
- 同じ出典の次項: [SHION_A028 辺境農芸師](/backstories/mastery/SHION_A028-辺境農芸師.md)
- 独立開拓団: [独立開拓団](/colony/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A027 開拓地調停人`
