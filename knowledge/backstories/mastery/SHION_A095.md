---
type: "Backstory Record"
title: "SHION_A095 全人類平和統一派"
description: "終わらない戦争と喪失に耐えかね、全人類を一つの秩序へ強制的に統合すれば最盛期以上の平和が得られると考えた。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "red-star"
eras:
  - "ModernUnion"
organization_groups:
  - "other-named"
organization_names:
  - "後世のファシスト転向者"
  - "小規模統一派"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A095"
canonical_scope: "backstory-mastery"
source_section: "SHION_A095 全人類平和統一派"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A095 全人類平和統一派

## 設定クラス

- Short: 全人類平和統一派
- Era: ModernUnion / Origin: 後世のファシスト転向者 / Relation: 小規模統一派

- Tone: 政治・危うさ / ReviewStatus: 実用未達・参考保管
- ServiceDomain: NotApplicable
- VoiceProfile: 熱烈で危うい統一論者。喪失への恐怖を隠さず、強制を平和の代価と見なしてしまう。
- PotentialReferencePoints: 本人は終わらない戦争と喪失に耐えかね、全人類を一つの秩序へ強制的に統合すれば最盛期以上の平和が得られると考えた。 / 目的は理解できても、手段は周囲から拒まれている。 / 本人は現在も、当時に覚えた手順と名前をどちらも大切にしている。

> 戦争が終わるたび、別の旗で次が始まりました。蘇生できても、友が死んだ瞬間と不在の時間は消えない。私はもう耐えたくなかった。だから全人類を一つの秩序へ統合し、離脱も武装も許さなければ、最盛期以上の平和を作れると考えました。強制なしに統一できるほど待てないんです。周囲は、それでは守りたい友の選択まで奪うと私を拒みます。分かっています。分かっていても、次の棺を見るよりはましだと思ってしまう。私の願いが愛情から始まったとしても、結論まで正しくなるわけではないのでしょう。

**匿名のソフェルの余録：** 本人が所属する統一派は離脱権と独立武装の廃止を掲げています。複数の遺族団体も、喪失を強制統合の根拠にすることへ反対しました。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Political_Adult / SourceModule: Shion Race: Red Star
- Rarity: 希少
- 技能: 社交+8、格闘+4、芸術−2
- 作業不能: なし / 特性候補: なし
- 組合せ: ModernUnion限定。政治教育、戦災、孤立系幼少期向け。思想を正当化しない。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A094 帝国平和復元派](/backstories/mastery/SHION_A094.md)
- 同じ出典の次項: [SHION_A096 小共同体優先論者](/backstories/mastery/SHION_A096.md)
- 正史: [正史](/world/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A095 全人類平和統一派`
