---
type: "Backstory Record"
title: "SHION_A355 律法院の下級書記"
description: "俺が律法院の基礎を作った一人だなんて話は誰が広めたのか知らないが、訂正させてくれ。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "canon"
eras:
  - "JourneyEra"
organization_groups:
  - "sofer-law"
organization_names:
  - "律法院"
  - "律法学舎出身"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A355"
canonical_scope: "backstory-mastery"
source_section: "SHION_A355 律法院の下級書記"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A355 律法院の下級書記

## 設定クラス

- Short: 律法院書記
- Era: JourneyEra / Origin: 律法学舎出身 / Relation: 律法院

- Tone: 律法・記録・抵抗 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: Civilian
- VoiceProfile: 淡々とした書記。誰かの美談扱いに強く反発する。
- PotentialReferencePoints: 新規稿のため該当なし。

> 俺が律法院の基礎を作った一人だなんて話は誰が広めたのか知らないが、訂正させてくれ。俺は下級書記で、上の人間が決めた条文を清書していただけだ。意見を求められたことは数えるほどしかない。それでも清書のたびに誤字を直し、矛盾する条文を上に報告したのは事実だ。基礎を作ったんじゃない、基礎の粗を拾っただけだ。立派な創設者みたいに語られるのは、正直居心地が悪い。ただの書記として名前を残してくれれば十分だ。

**匿名のソフェルの聞き取り：** 「創設者と呼ばれる気分は？」 — 「居心地が悪い」

## 実装予約

- Slot: Adulthood
- Pool: Shion_Journey_Adult / SourceModule: 律法の芽
- Rarity: 希少
- 技能: 知力+5、社交-1
- 作業不能: なし / 特性候補: なし
- 組合せ: 同一Eraの幼少期を優先。EraIndependentとの接続時は成人期側の年代を採用。
- 制作注記: 拡張稿（§12 Phase A超過分）。高補正階級=0。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A354 秩序維持隊の初代隊員](/backstories/mastery/SHION_A354-秩序維持隊の初代隊員.md)
- 同じ出典の次項: [SHION_A356 甲板治安班の巡回員](/backstories/mastery/SHION_A356-甲板治安班の巡回員.md)
- 正史: [正史](/world/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A355 律法院の下級書記`
