---
type: "Backstory Record"
title: "SHION_A054 古服再生師"
description: "持主が何度も身体を替えた後も、最初の外套を直し続けた。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
eras:
  - "EraIndependent"
organization_groups:
  - "other-named"
organization_names:
  - "衣装修復工房"
  - "無党派"
status: stable
authority: catalog
knowledge_role: catalog-record
canon_review: accepted
granularity: record
canonical_for: "backstories/mastery/SHION_A054"
canonical_scope: "backstory-mastery"
source_section: "SHION_A054 古服再生師"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A054 古服再生師

## 設定クラス

- Short: 古服再生師
- Era: EraIndependent / Origin: 無党派 / Relation: 衣装修復工房

- Tone: 工芸・記憶 / ReviewStatus: 採用済み
- ServiceDomain: NotApplicable
- VoiceProfile: 物静かな仕立師。素材の性能より、身についた癖と持主の記憶を丁寧に語る。
- PreservedCanonPoints: 本人は持主が何度も身体を替えた後も、最初の外套を直し続けた。 / 新素材なら一瞬で作れたが、擦れた袖口を残すため手縫いを選んだ。 / この経験は今も本人の判断に残り、新しい任務の折に静かに顔を出す。

> その外套の持主は、何度も身体を替えました。肩幅も腕の長さも変わるたび、私は同じ布をほどいて縫い直したんです。新素材で作れば一瞬で、もっと丈夫にもできました。でも持主は、擦れた袖口へ指を置いて「ここは残して」と言った。古い友人と並んで歩いた時についた跡だそうです。私は手で糸を通し、見えない裏側だけを補強しました。服は身体を守る道具ですが、身体が変わった後に昔の自分へ触れる場所にもなる。だから今も、直す前にどの傷を残したいか尋ねます。

**匿名のソフェルの余録：** 修復台帳には持主の身体変更ごとの採寸が残っています。袖口の摩耗部分だけは交換せず、裏面から補強する方法が一貫して用いられました。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Culture_Adult / SourceModule: CoreMOD
- Rarity: 一般
- 技能: 工芸+7、芸術+3、射撃−2
- 作業不能: なし / 特性候補: なし
- 組合せ: 全時代。工芸、芸術、故郷保存系幼少期向け。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A053 時間差郵便局長](/backstories/mastery/SHION_A053.md)
- 同じ出典の次項: [SHION_A055 動物終生保護官](/backstories/mastery/SHION_A055.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A054 古服再生師`
