---
type: "Backstory Record"
title: "SHION_A084 多重勲章の親衛兵"
description: "射撃と近接の双方で同族から勲章を受け、多数の暗殺と災害から評議員を守った。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
eras:
  - "EraIndependent"
organization_groups:
  - "empire"
  - "union"
organization_names:
  - "同盟・帝国混成"
  - "評議会親衛隊"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A084"
canonical_scope: "backstory-mastery"
source_section: "SHION_A084 多重勲章の親衛兵"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A084 多重勲章の親衛兵

## 設定クラス

- Short: 多重勲章親衛兵
- Era: EraIndependent / Origin: 同盟・帝国混成 / Relation: 評議会親衛隊

- Tone: 戦闘・護衛 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: Mixed
- VoiceProfile: 無駄口の少ない親衛兵。戦闘功績より避難者を守った判断を誇り、借り物を律儀に保管する。
- PotentialReferencePoints: 本人は射撃と近接の双方で同族から勲章を受け、多数の暗殺と災害から評議員を守った。 / 命令より避難者を優先した一件を、最も誇る叙勲理由としている。 / 長い時代が過ぎても、本人はこの出来事を古びた過去として扱わない。

> 射撃と近接の両方で章を受けました。評議員を狙う暗殺者も止め、崩落した議場から護衛対象を運び出した。ですが、いちばん誇るのは命令へ背いた一件です。退路を確保せよと言われた時、通路には避難できない市民が残っていました。私は評議員を安全区画へ押し込み、市民の列へ戻った。後で処分ではなく叙勲になったのは、守られた評議員自身が命令の誤りを認めたからです。展示棚には章より、避難者から借りた傘が多い。返そうとしましたが、「次も誰かへ使え」と言われましてね。

**匿名のソフェルの余録：** 射撃章と近接戦闘章に加え、避難者保護による特別章が確認されています。命令変更は護衛対象だった評議員自身の証言で承認されました。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Council_Adult / SourceModule: CoreMOD
- Rarity: 多重叙勲級
- 技能: 射撃+12、格闘+12、社交+6
- 作業不能: なし / 特性候補: なし
- 組合せ: 極希少な親衛兵。軍務、鉄律、公共奉仕系幼少期向け。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A083 百回帰還の外交官](/backstories/mastery/SHION_A083-百回帰還の外交官.md)
- 同じ出典の次項: [SHION_A085 独立星港建設者](/backstories/mastery/SHION_A085-独立星港建設者.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A084 多重勲章の親衛兵`
