---
type: "Backstory Record"
title: "SHION_A007 動物機工"
description: "荷獣の鞍、義肢、自動給餌器を同じ工房で整備する動物機工となった。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
  - "kombinat"
eras:
  - "EraIndependent"
organization_groups:
  - "other-named"
organization_names:
  - "牧畜工房"
  - "無党派"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A007"
canonical_scope: "backstory-mastery"
source_section: "SHION_A007 動物機工"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A007 動物機工

## 設定クラス

- Short: 動物機工
- Era: EraIndependent / Origin: 無党派 / Relation: 牧畜工房

- Tone: 動物・機械 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: NotApplicable
- VoiceProfile: 陽気で辛抱強い機工。役所の分類より、動物がまた歩けるかを気にする。
- PotentialReferencePoints: 本人は荷獣の鞍、義肢、自動給餌器を同じ工房で整備する動物機工となった。 / 動物が工具を怖がる時は、修理を急がず匂いと音へ慣らしてから作業した。 / 装具の調整では歩き方と皮膚の状態を観察し、飼育者にも保守方法を教えた。

> 獣医か機械工か、と役所には何度も聞かれました。患者は資格証を読みません。私は荷獣の鞍、義肢、自動給餌器を同じ工房で直しました。工具を怖がる動物には、匂いと音へ慣れるまで何日でも待ちます。歩き方と皮膚を見て装具を削り、飼育者へ保守も教えました。理論を説明するより、本人が歩く姿を見る方が判断しやすかったですね。必要だからだけではありません。友と調整の腕を競い、動物が昨日より一歩多く歩くのを見るのが好きでした。役に立つ趣味になったのは、少し癪ですが。

**匿名のソフェルの余録：** 工房台帳は鞍、義肢、給餌器を同じ利用者単位で管理しています。調整後の歩行と皮膚状態も記録され、飼育者向け保守票が添付されました。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Common_Adult / SourceModule: Shion Race: Kombinat
- Rarity: 専門
- 技能: 動物+6、工芸+3、知力−1
- 作業不能: なし / 特性候補: なし
- 組合せ: 全時代。動物義肢・牧畜・巡回修理系幼少期と好相性。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A006 移動診療医](/backstories/mastery/SHION_A006-移動診療医.md)
- 同じ出典の次項: [SHION_A008 構造聴診士](/backstories/mastery/SHION_A008-構造聴診士.md)
- Kombinat領域: [Kombinat領域](/kombinat/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A007 動物機工`
