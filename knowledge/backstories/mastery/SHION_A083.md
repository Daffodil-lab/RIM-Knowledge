---
type: "Backstory Record"
title: "SHION_A083 百回帰還の外交官"
description: "敵対圏へ何度も戻り、古い友人と新しい政府の双方へ話した。"
tags:
  - "shion"
  - "backstory"
  - "mastery"
eras:
  - "ModernUnion"
organization_groups:
  - "empire"
  - "union"
organization_names:
  - "帝国離反者系"
  - "同盟外務評議会"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "backstories/mastery/SHION_A083"
canonical_scope: "backstory-mastery"
source_section: "SHION_A083 百回帰還の外交官"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_A083 百回帰還の外交官

## 設定クラス

- Short: 百回帰還外交官
- Era: ModernUnion / Origin: 帝国離反者系 / Relation: 同盟外務評議会

- Tone: 外交・帰郷 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: NotApplicable
- VoiceProfile: 人懐こい老練な外交官。離反の理由と故郷への愛着を矛盾させず、玄関先の失敗も笑う。
- PotentialReferencePoints: 本人は敵対圏へ何度も戻り、古い友人と新しい政府の双方へ話した。 / 帝国を去った理由を忘れず、帝国で守られた暮らしまで嘘だったとは言わなかった。 / 本人は現在も、当時に覚えた手順と名前をどちらも大切にしている。

> 帝国を離反した後も、敵対圏へ何度も戻りました。古い友人とも、新しい政府とも話すためです。去った理由は忘れていません。内戦後の違法命令を許す気もない。それでも帝国で守られていた食卓や、律法を信じて働いた人々まで嘘だったとは言いたくありませんでした。帰還は記録上百回だそうです。二回は友人の玄関まで行って、呼鈴を押せず帰りました。あれを数えるかで書記と揉めています。外交官らしくない？　国家より、古い友へ何と名乗るかの方が難しい日もあるんですよ。

**匿名のソフェルの余録：** 外交通行記録は百件ありますが、うち二件は政府施設へ到達していません。本人は訪問未遂、書記は敵対圏への帰還として数えています。

## 実装予約

- Slot: Adulthood
- Pool: Shion_Council_Adult / SourceModule: CoreMOD
- Rarity: 功労
- 技能: 社交+10、知力+7、射撃+3
- 作業不能: なし / 特性候補: なし
- 組合せ: ModernUnion。帝国、旅行、討論系幼少期向け。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [backstories/mastery](/backstories/mastery/index.md)
- 同じ出典の前項: [SHION_A082 星域救難総監](/backstories/mastery/SHION_A082.md)
- 同じ出典の次項: [SHION_A084 多重勲章の親衛兵](/backstories/mastery/SHION_A084.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_A083 百回帰還の外交官`
