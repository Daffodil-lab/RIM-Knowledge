---
type: "Backstory Record"
title: "SHION_C359 今日から遡って話します"
description: "今日から遡って話しますでは、今日から遡って話しましょう。"
tags:
  - "shion"
  - "backstory"
  - "formation"
eras:
  - "JourneyEra"
organization_groups:
  - "empire"
  - "union"
organization_names:
  - "帝国准民"
  - "統治評議会随伴"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "reference/backstories/formation/SHION_C359"
canonical_scope: "backstory-formation"
source_section: "SHION_C359 今日から遡って話します"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_C359 今日から遡って話します

## 設定クラス

- Short: 遡る記憶
- Era: JourneyEra / Origin: 帝国准民 / Relation: 統治評議会随伴

- Tone: 回顧・逆順・成立 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: Military
- VoiceProfile: 落ち着いた老成した語りで、時間を遡って話す癖がある。
- PotentialReferencePoints: 新規稿のため該当なし。

> 今日から遡って話しましょう。今の私は秩序維持の記録整理を任されていますが、その前は分隊の見回り役でした。その前は、まだ帝国という言葉が正式に使われる前の統治評議会に、子供として随伴していました。さらに遡れば、十歳の私は評議会の議事録を運ぶだけの使い走りでした。もっと遡れば、旧い出身圏の言葉しか話さない先達に手を引かれて、甲板を歩いていた記憶があります。遡るほど言葉も暮らしも今と違いますが、歩いていた甲板の感触だけは変わりません。

## 実装予約

- Slot: Childhood
- Pool: Shion_Journey_Child / SourceModule: 帝国前夜
- Rarity: 希少
- 技能: 知力+3、射撃+2
- 作業不能: なし / 特性候補: なし
- 組合せ: 同一Era後期の秩序維持・統治評議会随伴系の成人期と優先接続。
- 制作注記: 拡張稿（§12 Phase A超過分）。高補正階級=0。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/reference/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [reference/backstories/formation](/reference/backstories/formation/index.md)
- 同じ出典の前項: [SHION_C358 帝国最初の落第兵](/reference/backstories/formation/SHION_C358.md)
- 同じ出典の次項: [SHION_C360 甲板の錆と最初の家](/reference/backstories/formation/SHION_C360.md)
- 正史: [正史](/world/index.md)
- 制作・開示規則: [制作・開示規則](/authoring/index.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_C359 今日から遡って話します`
