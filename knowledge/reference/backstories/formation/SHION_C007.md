---
type: "Backstory Record"
title: "SHION_C007 採掘場の石彫り"
description: "小惑星採掘場で岩を切り出し、作業後に残る壁面へ航路標識や事故犠牲者の記念像を彫った。"
tags:
  - "shion"
  - "backstory"
  - "formation"
eras:
  - "EraIndependent"
organization_groups:
  - "unaffiliated-unknown"
organization_names:
  - "未形成"
  - "無党派"
status: stable
authority: catalog
knowledge_role: catalog-record
reference_review: reference-only
granularity: record
canonical_for: "reference/backstories/formation/SHION_C007"
canonical_scope: "backstory-formation"
source_section: "SHION_C007 採掘場の石彫り"
generated:
  by: "process:rim-okf-migration"
  at: "2026-07-26T00:54:52.353Z"
sources:
  - id: "migration-source"
    resource: "retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md"
    title: "シオン/Shion ゲーム用バックストーリー完成カタログ v3"
---

# SHION_C007 採掘場の石彫り

## 設定クラス

- Short: 石彫り
- Era: EraIndependent / Origin: 無党派 / Relation: 未形成

- Tone: 芸術・労働 / ReviewStatus: 実用未達・参考保管
- ServiceDomain: NotApplicable
- VoiceProfile: 粗野 / 力強い / 石への親近感 / 傷を誇示しない
- PotentialReferencePoints: 本人は小惑星採掘場で岩を切り出し、作業後に残る壁面へ航路標識や事故犠牲者の記念像を彫った。 / 採掘計画を妨げず、遠くからでも読める形を作る必要があった。 / 鉱脈と亀裂を避けながら彫るうち、石の性質を見極める技術と大きな構図を考える力を身につけた。

> 記念像？　そんな立派なものじゃない。採掘後の壁へ、帰路の矢印と死んだ連中の横顔を彫っただけです。鉱脈を潰せば怒鳴られるし、亀裂を跨げば像ごと割れる。だから石の癖を読み、遠くの作業灯でも鼻筋が見えるほど線を深くした。仲間は道標代わりだと笑いましたが、それでよかった。芸術は静かな部屋だけのものじゃないでしょう。掘り尽くした場所にも、誰が汗をかいたか残すべきだ。故郷を直せても、あの粉塵まみれの顔までは戻らない。私は傷を磨き消す趣味がないんです。

**匿名のソフェルの余録：** 小惑星の彫刻は近くでは全体が見えません。作業艇で離れて初めて読めるので、鑑賞には少々燃料が要ります。切削記録には観覧軌道からの確認工程もあります。

## 実装予約

- Slot: Childhood
- Pool: Shion_Common_Child / SourceModule: CoreMOD
- Rarity: 専門
- 技能: 採掘+5、芸術+4、社交−2
- 作業不能: なし / 特性候補: なし
- 組合せ: 全時代。芸術・採掘・宇宙系成人期向け。
- 制作注記: 旧稿の出来事と技能根拠を保持して口述化。

## 関連項目

- 設定と実装の分離: [設定本文と実装予約の分離](/reference/backstories/09-設定本文と実装予約の分離.md)
- 上位索引: [reference/backstories/formation](/reference/backstories/formation/index.md)
- 同じ出典の前項: [SHION_C006 記念庭園の園丁](/reference/backstories/formation/SHION_C006.md)
- 同じ出典の次項: [SHION_C008 巡回修理屋](/reference/backstories/formation/SHION_C008.md)

## 出典

- シオン/Shion ゲーム用バックストーリー完成カタログ v3（退役済み原本: `retired-source://project/docs/38_SHION_BACKSTORY_CATALOG.md`） — `SHION_C007 採掘場の石彫り`
