...
1. `Codex接続`が`initialize`、`initialized`、`model/list`を実行します。
...
- `public/`: dark node graph、inspector、検証、二種類の承認、review、event表示。
...
- event stream: token、goal、turn、hookなどのlifecycle証拠を残し、agent message deltaはGUIと保存stateの両方から除外します。
